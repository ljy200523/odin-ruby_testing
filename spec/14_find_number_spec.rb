...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        expect(game_guessing.make_guess(0, 9)).to eq(4)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        # Use number_guessing as a dummy to instantiate FindNumber since currently don't have RandomNumber class yet, resulting in an error w/o the dummy
        game = described_class.new(5, 9, number_guessing)
        expect(game.make_guess(5, 9)).to eq(7)
      end
    end
    ...
    context 'when min is 8 and max is 9' do
      it 'returns 8' do
        min = 8
        max = 9
        game = described_class.new(min, max, number_guessing)
        expect(game.make_guess(min, max)).to eq(8)
      end
    end
    ...
    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        ...
        max = 3
        game = described_class.new(min, max, number_guessing)
        expect(game.make_guess(min, max)).to eq(1)
      end
    end
    ...
    context 'when min and max both equal 3' do
      it 'returns 3' do
        ...
        max = 3
        game = described_class.new(min, max, number_guessing)
        expect(game.make_guess(min, max)).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game_won) { described_class.new(0, 9, random_number, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double("random_number", value: 8) }
      subject(:game_not_won) { described_class.new(0, 9, random_number, 1) }

      it 'is not game over' do
        expect(game_not_won).to_not be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect(low_guess_game.update_range).to eq([5, 9])
      end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        expect(high_guess_game.update_range).to eq([0, 8])
      end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect(edge_case.update_range).to eq([8, 8])
      end
