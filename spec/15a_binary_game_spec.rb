...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_incorrect_value = "a"
        second_incorrect_value = "b"
        valid_input = "2"
        allow(game_input).to receive(:gets).and_return(first_incorrect_value, second_incorrect_value, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        valid_input = 8
        expect(new_game.verify_input(1, 10, valid_input)).to eq(8)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        invalid_input = 211
        expect(new_game.verify_input(1, 10, invalid_input)).to eq(nil)
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      ...
      it 'returns 9' do
        max = game_hundreds.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect{ new_game.display_turn_order(binary_search) }.to change{ new_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess)
      new_game.display_turn_order(binary_search)
    end
