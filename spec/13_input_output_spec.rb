...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continue) { described_class.new(8, '1') }
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        user_input = '29'
        verified = game_check.verify_input(user_input)
        expect(verified).to eq(nil)
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        incorrect_two = 'laze'
        valid_input = '3'
        allow(game_loop).to receive(:player_input).and_return(incorrect_one, incorrect_two, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_in_4) { described_class.new(5, '5', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_in_4.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
end
